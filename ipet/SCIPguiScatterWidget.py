...
from Tkinter import StringVar, Label, Frame, Canvas, Tk, Toplevel
...
class SCIPguiScatterWidget(SCIPguiWidget):
   '''
   classdocs
   '''
   name="Scatter Plot"
...
#      matplotlib.artist.setp(thisline, color=colors)
      ...
      self.canvas.draw()
      self.pointwindow = Toplevel()
      self.pointwindow.wm_title("Selected Points")
      self.pointwindow.geometry("200x100+%d+%d"%(event.mouseevent.x, event.mouseevent.y))
      for elem in points:
