...
class PrimalBoundHistoryReader(StatisticReader):
   ...
   easyCPLEX = True
   totalnumberofsols = 0
   heurdispcharexp = re.compile('^[a-zA-Z*]')
   ...
   def extractStatistic(self, line):
      if StatisticReader.solvertype == StatisticReader.SOLVERTYPE_SCIP:
         ...
         elif self.inTable and self.shorttablecheckexp.search(line) and self.heurdispcharexp.match(line):
   #         print line
            ...
            if PrimalBound != self.lastPrimalBound:
               ...
               try:
                   ...
                   # we just don't append in case that something can't be parsed correctly
                   pass
