...
class ScrolledOutputWidget(Frame):
    ...
    def setProblem(self, problemname):
        self.selectedproblem = problemname

    def setTestrun(self, testrun):
        self.selectedtestrun = testrun
    ...
    def update(self):
        ...
        try:
            f = open(self.selectedtestrun.filenames[0], 'r')
        except:
            return
        ...
        for line in f:
            if not rightProblem and line.startswith('@01') and self.selectedproblem in line:
                rightProblem = True
                self.inOutput = True
            if rightProblem and line.startswith("SCIP Status        :"):
                self.inOutput = False
